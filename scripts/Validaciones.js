				...
				switch (tipoinput) {
					case "email":
						if (!email_filter.test(valor) || valor.match(email_illegalChars)) {
							...
							$(this).parents(".tooltips").siblings(".spanValidacion").toggleClass('hidden',false).text(mensajeAlerta);
							$(this).parents(".tooltips").find(".spanValidacion").toggleClass("hidden", false)
							$("html,body").animate({ scrollTop: $(this).parents(".tooltips").siblings(".spanValidacion").offset().top - 80 }, 200, "swing", function () {
								$(this).focus();
							});
							return false;
						} else {
...
function validarlenghtcampo(elemento, valor, titulo, minimo, maximo, tipocaracteres) {
	...
	if (typeof minimoCaracteres !== typeof undefined && minimoCaracteres !== false && typeof maximoCaracteres !== typeof undefined && maximoCaracteres !== false) {
		if (minimoCaracteres == maximoCaracteres && numeroCaracteres != minimoCaracteres) {
			...
			$(elemento).siblings(".spanValidacion").toggleClass("hidden", false)
...
function mostrarvalidaciontitle(input) {
	...
	switch (tipoInput) {
		case "text":
			...
			$(input).parents(".tooltips").find(".spanValidacion").toggleClass("hidden", false)
...
function validarsummernote(id) {
	...
	if (code) {
		...
		$(id).parents(".tooltips").find(".spanValidacion").toggleClass("hidden", false)
		return false;
	} else {
	}
	if (banderaRespuesta == true) {
...
function validarcontrasenias(elemento1, elemento2) {
	...
	if (original === replica) {
		...
		$(elemento2).parents(".tooltips").siblings(".spanValidacion").toggleClass('hidden',false).text(mensajeAlerta);
		$(elemento2).parents(".tooltips").find(".spanValidacion").toggleClass("hidden", false)
